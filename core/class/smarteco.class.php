...
class smarteco extends eqLogic {
	...
	public function syncWithSmartEco() {
		...
		foreach ($pieces as $piece) {
			...
			if (!is_object($eqLogic) && $thermostat != 'NULL') {
				...
				if(isset($pieceParent)) $eqLogic->setObject_id($pieceParent->getId());
	...
	protected function createCommand($eqLogic, $cmdId, $type, $subType, $name, $visible = 1, $value = '', $order = 999, $dashboard ='', $mobile = '', $isHistorized = 0, $icon = null, $generic_type = null)
	{
		...
		if (!is_object($cmd)) $cmd = new smartecoCmd();
		...
		if ($dashboard != '') $cmd->setTemplate('dashboard', $dashboard);
		else $cmd->setTemplate('dashboard', 'default');
		
		if ($mobile != '') $cmd->setTemplate('mobile', $mobile);
		else $cmd->setTemplate('mobile', 'default');
